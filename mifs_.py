"""
Note: all rights retained by the original author, not licensed under MIT like the rest of the repo.
VERY lightly modified (due to a few bugs and deprecations) from:
...
class MutualInformationFeatureSelector(BaseEstimator, SelectorMixin):
    ...
    def __init__(self, method='JMI', k=5, n_features='auto', categorical=True,
                 subsample=5_000, n_jobs=1, verbose=0, random_state=42):
        self.method = method
        self.k = k
    ...
    def fit(self, X, y):
        ...
        if X.shape[0] > self.subsample:
            X = pd.DataFrame(X).copy().sample(self.subsample, random_state=self.random_state).values
            y = pd.Series(y).copy().sample(self.subsample, random_state=self.random_state).values
